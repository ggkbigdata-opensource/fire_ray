...
<div id="tblQuery" style="padding:3px">
	...
	<div class='inputDivEg'>
		<span style="font-size:12px">编码:</span>
		<input id="code" style="line-height:20px;" class="input">
	</div>
	<div class='inputDivEg'>
		<span style="font-size:12px">说明:</span>
		<input id="remark" style="line-height:20px;" class="input">
	</div>
    <div class='inputDivEg'>
...
<table id="dg" title="数据列表" class="easyui-datagrid" style="width:auto;height:auto;"
        url='<%=request.getContextPath() %>/app/buildingFloor/queryPage'
        		toolbar="#toolbar"
       			rownumbers="true" 
        		fitColumns="true" 
        		singleSelect="false"
...
<div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
    ...
    <div class="ftitle">BuildingFloor Information</div>
    <form id="fm_building_floor" method="post" novalidate>
        <div class="fitem">
            <label>楼层名称:</label>
        ...
        <div class="fitem">
            <label>建筑:</label>
            <input id="comCc"  name = "buildingId"  class="easyui-combobox" 
...
var UrlConfig = {
		listPage: '<%=request.getContextPath() %>/app/buildingFloor/queryPage',
		insertData: '<%=request.getContextPath() %>/app/buildingFloor/insertData',
		updateData: '<%=request.getContextPath() %>/app/buildingFloor/updateData',
		deleteData: '<%=request.getContextPath() %>/app/buildingFloor/deleteData',
		deleteDataByIds: '<%=request.getContextPath() %>/app/buildingFloor/deleteDataByIds',
		exportLibStreet: '<%=request.getContextPath() %>/app/buildingFloor/exportLibStreet',
		importStreet: '<%=request.getContextPath() %>/app/buildingFloor/importStreet',
};
...
// 新增
function newStreet(){
    $('#dlg').dialog('open').dialog('setTitle','New building');
    $('#fm_building_floor').form('clear');
    url =UrlConfig.insertData;
}

// 编辑修改
function editStreet(){
    var rowNum = $('#dg_building_floor').datagrid("getSelections").length;
    if(rowNum > 1 || rowNum == 0){
        $.messager.alert("提示","请选择一行修改！","error");
    }else {
        var row = $('#dg_building_floor').datagrid('getSelected');
        if (row) {
            $('#dlg').dialog('open').dialog('setTitle', 'Edit building');
            $('#fm_building_floor').form('load', row);
    ...
    $('#fm_building_floor').form('submit',{
        ...
        success: function(result){
            ...
            if (result.errorMsg){
                ...
                $('#dg_building_floor').datagrid('reload');    // reload the user data
            }
        }
    });
}
...
function destroyStreets(){//返回选中多行
    var selRow = $('#dg_building_floor').datagrid('getSelections')
    //判断是否选中行
    if (selRow.length==0) {
        ...
        $.messager.confirm('提示', '是否删除选中数据?', function (r) {
            if (!r) {
                ...
                function(result) {  
                          $('#dg_building_floor').datagrid('clearSelections');
                          $.messager.alert("提示", "恭喜您，信息删除成功！", "info");
                          $('#dg_building_floor').datagrid('reload');
                });
            }
        });
    }
...
function doSearch(value,name){
	$('#dg_building_floor').datagrid('load',{
		name: $('#name').val(),
		code: $('#code').val(),
		remark: $('#remark').val(),
		buildingId:$('#buildingId').combobox('getValue')
	});
}
