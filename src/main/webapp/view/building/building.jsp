...
<div>
	...
	<input id="blockId" class="easyui-combobox" 
	       data-options="width:150"> 
	<span style="font-size:12px">建筑类别:</span>
	<input id="baseBuildingClass" class="easyui-combobox" 
	       data-options="editable:false,width:150"> 
</div>
<div style="margin-top: 10px">
    <div class='inputDivEg'>
        <span style="font-size:12px">建筑级别:</span>
        <input id="baseLevel" class="easyui-combobox"
               data-options="editable:false,width:150">
    </div>
	<div class='inputDivEg'>
		<span style="font-size:12px">建筑名称:</span>
		<input id="baseName" class="easyui-textbox" style="line-height:20px;"/>
	</div>
...
<table id="dg_building" title="数据列表" class="easyui-datagrid" style="width:auto;height:auto;"
...
<div id="dlgBuilding" class="easyui-dialog" style="width:750px;height:560px;padding:10px 20px"
    ...
    <form id="fm_building" method="post" novalidate>
	    ...
	    <div class="fitem_building">
		    <label>建筑名称:</label>
		    <input name="baseName" class="easyui-textbox" required="true" />
		</div>
	     <div class="fitem_building">
			<label>建筑类别:</label>
		...
		<div class="fitem_building">
			<label>级别:</label>
			<input id="comCcBaseLevel"  name = "baseLevel"  class="easyui-combobox" 
				data-options="required:true,editable:false,width:173" /> 
		</div>
		<div class="fitem_building">
			<label>竣工时间:</label>
			<input class="easyui-datetimebox" name="finishTimeString"
	    ...
	    <div class="fitem_building">
	     ...
	     <div class="fitem_building">
		   ...
		   <div class="fitem_building">
	         <label>消防负责人电话:</label>
	         <input name="fireManagerPhone" class="easyui-textbox" data-options="validType:'length[1,11]'" prompt="请输入11位数字">
	      </div>
		  	<div class="fitem_building">
	         <label>消防联系人:</label>
	         <input name="fireContact" class="easyui-textbox">
	      </div>
		   <div class="fitem_building">
	         <label>消防联系人电话:</label>
	         <input name="fireContactPhone" class="easyui-textbox" data-options="validType:'length[1,11]'" prompt="请输入11位数字">
	      </div>
	      <div class="fitem_building">
				<label>工程类别:</label>
				<input id="comCcConType"  name = "conType"  class="easyui-combobox" 
					data-options="required:true,editable:false,width:173" /> 
		  </div>
			<div class="fitem_building">
	         <label>建筑高度:</label>
	         <input name="conBuildHight" class="easyui-numberbox" precision="0"/>
	      </div>
	      <div class="fitem_building">
	         <label>建筑面积:</label>
	         <input name="conBuildArea"class="easyui-numberbox" precision="2"/>
	      </div>
		   <div class="fitem_building">
	         <label>占地面积:</label>
	         <input name="conCoverArea" class="easyui-numberbox" precision="2"/>
	      </div>
		   <div class="fitem_building">
	         <label>地表层数:</label>
	         <input name="conFloors" class="easyui-numberbox" precision="0"/>
	      </div>
	      <div class="fitem_building">
	         <label>地下层数:</label>
	         <input name="conUnderFloors" class="easyui-numberbox" precision="0"/>
	      </div>
	        <div class="fitem">
	            <label class="area">地址:</label>
	            <input class="easyui-textbox" data-options="multiline:true" name="baseAddress" style="width:500px;height:70px">
         </div>
         <div class="fitem">
	         <label class="area">说明:</label>
	         <input class="easyui-textbox" data-options="multiline:true" name="baseRemark" style="width:500px;height:70px">
	     </div>
    ...
    <form id="file_building_img" method="post" enctype="multipart/form-data">
	    <div class="fitem_building" >
		    <label>上传缩略图:</label>
		    <input class="easyui-filebox" data-options="buttonText:'选择'" id="thumb_img_input" name="thumb_img" style="width:200px">
	    </div>
    </form>
         <div class="fitem" style="margin-top: 30px;margin-left: -335px;margin-bottom: 30px;">
...
<div id="dlg-buttons">
    ...
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlgBuilding').dialog('close');$('#showSelected_building').text('');" style="width:90px">取消</a>
</div>


<div id="import_building" class="easyui-dialog" style="width:300px;height:250px;padding:10px 20px"
    closed="true">
<div class="ftitle">导入数据</div>
<form id="fm_file_building" method="post" enctype="multipart/form-data" target="ajaxUpload">
		<div style="margin-top:10px;">
    		<a href="<%=request.getContextPath() %>/template/building_import_template.xls"   class="easyui-linkbutton" iconCls="icon-add" target='_blank'>导入模板下载</a>
    	</div>
    	<div style="margin-top:10px;">
...
// 编辑修改
function editStreet(){
	var thumbImgUrl= "http://"+window.location.host+$('#dg_building').datagrid("getSelections")[0].thumbImg;
    ...
    if(rowNum > 1 || rowNum == 0){
        ...
        if (row) {
            //表单加载数据前关掉联动查询
            disableLinkage(true);
            $('#dlgBuilding').dialog('open').dialog('setTitle', '编辑建筑主体');
...
function doExport(value,name){
	$.messager.confirm('提示', '是否导出查询出来数据?', function (r) {
        if (!r) {
        	...
        	"?baseName="+encodeURI(exportBaseName)+
